...
// ============================================================
// WHOLESALE DISTRIBUTION — WholesaleOS additions
// Ferguson-pattern: branch hierarchy, stocking policy, demand
// classification, contractor tiering, external signals, and
// multi-echelon network positioning results.
// ============================================================
...
export const branches = pgTable(
  'branches',
  {
    id: serial('id').primaryKey(),
    name: varchar('name', { length: 255 }).notNull().unique(),
    branchType: varchar('branch_type', { length: 30 }).notNull(), // 'central_dc' | 'regional_dc' | 'hub' | 'spoke'
    parentBranchId: integer('parent_branch_id'), // self-reference — populated after insert
    latitude: doublePrecision('latitude'),
    longitude: doublePrecision('longitude'),
    region: varchar('region', { length: 100 }),
    squareFootage: integer('square_footage'),
    contractorMixResidentialPct: numeric('contractor_mix_residential_pct', { precision: 5, scale: 2 }),
    contractorMixCommercialPct: numeric('contractor_mix_commercial_pct', { precision: 5, scale: 2 }),
    active: boolean('active').default(true),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow()
  },
  (table) => ({
    branchTypeCheck: sql`CHECK (${table.branchType} IN ('central_dc', 'regional_dc', 'hub', 'spoke'))`,
    branchTypeIdx: index('idx_branches_type').on(table.branchType),
    regionIdx: index('idx_branches_region').on(table.region),
    parentIdx: index('idx_branches_parent').on(table.parentBranchId),
    latLongIdx: index('idx_branches_lat_long').on(table.latitude, table.longitude)
  })
);
...
export const branchesRelations = relations(branches, ({ one, many }) => ({
  ...
  children: many(branches, { relationName: 'parent_child' }),
  skuPolicies: many(skuBranchPolicy),
  networkDecisions: many(networkPositioningDecisions)
}));

// --- Projects / job tickets -------------------------------------------------------
export const projects = pgTable(
  'projects',
  {
    id: serial('id').primaryKey(),
    customerId: integer('customer_id').notNull().references(() => customers.id, { onDelete: 'cascade' }),
    projectType: varchar('project_type', { length: 30 }).notNull(), // 'residential_new' | 'residential_reno' | 'commercial' | 'industrial' | 'maintenance'
  ...
  (table) => ({
    projectTypeCheck: sql`CHECK (${table.projectType} IN ('residential_new', 'residential_reno', 'commercial', 'industrial', 'maintenance'))`,
    customerIdIdx: index('idx_projects_customer_id').on(table.customerId),
    projectTypeIdx: index('idx_projects_type').on(table.projectType),
...
export const projectsRelations = relations(projects, ({ one, many }) => ({
  ...
  lineItems: many(projectLineItems)
}));
...
export const projectLineItems = pgTable(
  'project_line_items',
  {
    ...
    skuId: integer('sku_id').notNull().references(() => products.id, { onDelete: 'cascade' }),
    quantity: decimal('quantity', { precision: 15, scale: 4 }).notNull(),
    fulfillmentBranchId: integer('fulfillment_branch_id').references(() => branches.id, { onDelete: 'set null' }),
    fulfillmentDate: date('fulfillment_date'),
    createdAt: timestamp('created_at').defaultNow()
  },
  (table) => ({
    projectIdIdx: index('idx_project_line_items_project').on(table.projectId),
    skuIdIdx: index('idx_project_line_items_sku').on(table.skuId),
    branchIdIdx: index('idx_project_line_items_branch').on(table.fulfillmentBranchId)
  })
);
export const insertProjectLineItemSchema = createInsertSchema(projectLineItems);

export const projectLineItemsRelations = relations(projectLineItems, ({ one }) => ({
  project: one(projects, { fields: [projectLineItems.projectId], references: [projects.id] }),
  sku: one(products, { fields: [projectLineItems.skuId], references: [products.id] }),
  fulfillmentBranch: one(branches, { fields: [projectLineItems.fulfillmentBranchId], references: [branches.id] })
}));

// --- SKU-branch stocking policy ---------------------------------------------------
export const skuBranchPolicy = pgTable(
  'sku_branch_policy',
  {
    id: serial('id').primaryKey(),
    skuId: integer('sku_id').notNull().references(() => products.id, { onDelete: 'cascade' }),
    branchId: integer('branch_id').notNull().references(() => branches.id, { onDelete: 'cascade' }),
    stockingPolicy: varchar('stocking_policy', { length: 25 }).notNull(), // 'carry' | 'hub_replenish' | 'regional_replenish' | 'special_order' | 'drop_ship'
    targetServiceLevel: numeric('target_service_level', { precision: 5, scale: 4 }), // 0.0000–1.0000
    safetyStock: decimal('safety_stock', { precision: 12, scale: 4 }),
  ...
  (table) => ({
    stockingPolicyCheck: sql`CHECK (${table.stockingPolicy} IN ('carry', 'hub_replenish', 'regional_replenish', 'special_order', 'drop_ship'))`,
    uniqueSkuBranch: uniqueIndex('idx_sku_branch_policy_unique').on(table.skuId, table.branchId),
    policyIdx: index('idx_sku_branch_policy_policy').on(table.stockingPolicy),
    branchIdIdx: index('idx_sku_branch_policy_branch').on(table.branchId)
  })
);
export const insertSkuBranchPolicySchema = createInsertSchema(skuBranchPolicy);

export const skuBranchPolicyRelations = relations(skuBranchPolicy, ({ one }) => ({
  sku: one(products, { fields: [skuBranchPolicy.skuId], references: [products.id] }),
  branch: one(branches, { fields: [skuBranchPolicy.branchId], references: [branches.id] })
}));

// --- Contractor tiering -----------------------------------------------------------
export const customerTier = pgTable(
  'customer_tier',
  {
    id: serial('id').primaryKey(),
    customerId: integer('customer_id').notNull().unique().references(() => customers.id, { onDelete: 'cascade' }),
    tier: varchar('tier', { length: 20 }).notNull(), // 'platinum' | 'gold' | 'silver' | 'transactional'
    serviceLevelTarget: numeric('service_level_target', { precision: 5, scale: 4 }).notNull(), // e.g. 0.9900
    annualRevenue: decimal('annual_revenue', { precision: 15, scale: 2 }),
    primaryBranchId: integer('primary_branch_id').references(() => branches.id, { onDelete: 'set null' }),
    tierAssignedAt: timestamp('tier_assigned_at').defaultNow(),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow()
  },
  (table) => ({
    tierCheck: sql`CHECK (${table.tier} IN ('platinum', 'gold', 'silver', 'transactional'))`,
    tierIdx: index('idx_customer_tier_tier').on(table.tier),
    primaryBranchIdx: index('idx_customer_tier_branch').on(table.primaryBranchId)
  })
);
export const insertCustomerTierSchema = createInsertSchema(customerTier);

export const customerTierRelations = relations(customerTier, ({ one }) => ({
  customer: one(customers, { fields: [customerTier.customerId], references: [customers.id] }),
  primaryBranch: one(branches, { fields: [customerTier.primaryBranchId], references: [branches.id] })
}));

// --- SKU demand classification ----------------------------------------------------
export const skuDemandProfile = pgTable(
  'sku_demand_profile',
  {
    id: serial('id').primaryKey(),
    skuId: integer('sku_id').notNull().unique().references(() => products.id, { onDelete: 'cascade' }),
    adi: numeric('adi', { precision: 10, scale: 4 }), // Average Demand Interval
    cvSquared: numeric('cv_squared', { precision: 10, scale: 6 }), // Coefficient of Variation²
    classification: varchar('classification', { length: 20 }).notNull(), // 'smooth' | 'intermittent' | 'erratic' | 'lumpy'
    recommendedMethod: varchar('recommended_method', { length: 20 }).notNull(), // 'croston' | 'sba' | 'tsb' | 'lightgbm' | 'ets'
    lastClassifiedAt: timestamp('last_classified_at').defaultNow(),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow()
  },
  (table) => ({
    classificationCheck: sql`CHECK (${table.classification} IN ('smooth', 'intermittent', 'erratic', 'lumpy'))`,
    methodCheck: sql`CHECK (${table.recommendedMethod} IN ('croston', 'sba', 'tsb', 'lightgbm', 'ets'))`,
    classificationIdx: index('idx_sku_demand_profile_class').on(table.classification),
    methodIdx: index('idx_sku_demand_profile_method').on(table.recommendedMethod)
  })
);
export const insertSkuDemandProfileSchema = createInsertSchema(skuDemandProfile);

export const skuDemandProfileRelations = relations(skuDemandProfile, ({ one }) => ({
  sku: one(products, { fields: [skuDemandProfile.skuId], references: [products.id] })
}));

// --- External signals (leading indicators) ----------------------------------------
export const externalSignals = pgTable(
  'external_signals',
  {
    id: serial('id').primaryKey(),
    signalType: varchar('signal_type', { length: 50 }).notNull(), // 'housing_starts' | 'building_permits' | 'construction_spending' | 'weather_forecast'
    region: varchar('region', { length: 100 }).notNull(),
    period: varchar('period', { length: 20 }).notNull(), // e.g. '2024-W04', '2024-03'
    value: decimal('value', { precision: 20, scale: 6 }).notNull(),
    source: varchar('source', { length: 100 }), // 'census_bureau' | 'noaa' | 'manual'
    loadedAt: timestamp('loaded_at').defaultNow(),
    createdAt: timestamp('created_at').defaultNow()
  },
  (table) => ({
    signalTypeCheck: sql`CHECK (${table.signalType} IN ('housing_starts', 'building_permits', 'construction_spending', 'weather_forecast'))`,
    uniqueSignalPeriodRegion: uniqueIndex('idx_ext_signals_unique').on(table.signalType, table.region, table.period),
    signalTypeIdx: index('idx_ext_signals_type').on(table.signalType),
    regionIdx: index('idx_ext_signals_region').on(table.region),
    periodIdx: index('idx_ext_signals_period').on(table.period)
  })
);
export const insertExternalSignalSchema = createInsertSchema(externalSignals);

// --- Multi-echelon network positioning scenarios ---------------------------------
export const networkPositioningScenarios = pgTable(
  'network_positioning_scenarios',
  {
    id: serial('id').primaryKey(),
    name: varchar('name', { length: 255 }).notNull(),
    createdAt: timestamp('created_at').defaultNow(),
    objectiveValue: decimal('objective_value', { precision: 20, scale: 6 }),
    totalInventoryCost: decimal('total_inventory_cost', { precision: 20, scale: 2 }),
    avgFillRate: numeric('avg_fill_rate', { precision: 6, scale: 4 }), // 0.0000–1.0000
    solveTimeSeconds: decimal('solve_time_seconds', { precision: 10, scale: 3 }),
    status: varchar('status', { length: 20 }).default('pending'), // 'pending' | 'running' | 'complete' | 'failed'
    notes: text('notes'),
    updatedAt: timestamp('updated_at').defaultNow()
  },
  (table) => ({
    nameIdx: index('idx_net_pos_scenario_name').on(table.name),
    statusIdx: index('idx_net_pos_scenario_status').on(table.status),
    createdIdx: index('idx_net_pos_scenario_created').on(table.createdAt)
  })
);
export const insertNetworkPositioningScenarioSchema = createInsertSchema(networkPositioningScenarios);

export const networkPositioningScenariosRelations = relations(networkPositioningScenarios, ({ many }) => ({
  decisions: many(networkPositioningDecisions)
}));

// --- Multi-echelon network positioning decisions ---------------------------------
export const networkPositioningDecisions = pgTable(
  'network_positioning_decisions',
  {
    id: serial('id').primaryKey(),
    scenarioId: integer('scenario_id').notNull().references(() => networkPositioningScenarios.id, { onDelete: 'cascade' }),
    skuId: integer('sku_id').notNull().references(() => products.id, { onDelete: 'cascade' }),
    branchId: integer('branch_id').notNull().references(() => branches.id, { onDelete: 'cascade' }),
    policy: varchar('policy', { length: 25 }).notNull(), // 'carry' | 'hub_replenish' | 'regional_replenish' | 'special_order' | 'drop_ship'
    stockLevel: decimal('stock_level', { precision: 12, scale: 4 }),
    flowFromBranchId: integer('flow_from_branch_id').references(() => branches.id, { onDelete: 'set null' }),
    createdAt: timestamp('created_at').defaultNow()
  },
  (table) => ({
    policyCheck: sql`CHECK (${table.policy} IN ('carry', 'hub_replenish', 'regional_replenish', 'special_order', 'drop_ship'))`,
    scenarioIdIdx: index('idx_net_pos_dec_scenario').on(table.scenarioId),
    skuIdIdx: index('idx_net_pos_dec_sku').on(table.skuId),
    branchIdIdx: index('idx_net_pos_dec_branch').on(table.branchId),
    uniqueScenarioSkuBranch: uniqueIndex('idx_net_pos_dec_unique').on(table.scenarioId, table.skuId, table.branchId)
  })
);
export const insertNetworkPositioningDecisionSchema = createInsertSchema(networkPositioningDecisions);

export const networkPositioningDecisionsRelations = relations(networkPositioningDecisions, ({ one }) => ({
  scenario: one(networkPositioningScenarios, { fields: [networkPositioningDecisions.scenarioId], references: [networkPositioningScenarios.id] }),
  sku: one(products, { fields: [networkPositioningDecisions.skuId], references: [products.id] }),
  branch: one(branches, { fields: [networkPositioningDecisions.branchId], references: [branches.id] }),
  flowFromBranch: one(branches, {
    fields: [networkPositioningDecisions.flowFromBranchId],
    references: [branches.id],
    relationName: 'flow_source'
  })
}));
